export class Modal {
    ...
    createFooter() {
        ...
        accept.addEventListener('click', () => {
            ...
            if (userName.length >= 5 && userName.match(/[a-zA-Z]/)) {
                if (password.length >= 8 && password.length <= 15) {
                    const validPassword = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?=.*[!._-]).{8,15}$/;
                    if (password.match(validPassword)) {
                        alert('Welcome to our website. Enjoy!');
                        this.close(Modal.ACCEPT);
                    } else {
                        alert('Your password must contain at least 1 lowercase alphabetical character, 1 uppercase alphabetical character and 1 of these characters: . _ ! -. Please try again.');
                    }
                } else {
                    alert("Password's lentgh should be 8-15 characters. Please try again.");
                };
            } else {
                alert('Username should have at least 5 alphabetical characters. Please try again.');
            }
        });
